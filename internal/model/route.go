...
type Route struct {
	...
	SegmentLengthM   int32      `gorm:"not null" json:"segment_length_m"`
	VideoFilename    string     `gorm:"type:varchar(255)" json:"video_filename"`
	VideoPath        string     `gorm:"type:varchar(500)" json:"video_path"`
	
	// Общая статистика
	TotalFrames         int32   `gorm:"not null" json:"total_frames"`
	TotalDistanceMeters float64 `gorm:"not null" json:"total_distance_meters"`
	TotalSegments       int32   `gorm:"not null" json:"total_segments"`
	SegmentsWithData    int32   `gorm:"not null" json:"segments_with_data"`
	AverageCoverage     float64 `gorm:"not null" json:"average_coverage"`
